...
in mkMerge [
	{
		"${optionalString (elem release [ "24.05" ]) release}" = mkIf nixosConfig.services.xserver.desktopManager.gnome.enable {
			dconf.settings = {
				# Background
		...
		"${optionalString (elem release [ "24.11" "24.11" "25.05" "25.11" ]) release}" = mkIf nixosConfig.services.xserver.desktopManager.gnome.enable {
			dconf.settings = {
				# Background
					"org/gnome/desktop/background" = {
						#picture-uri="${pkgs.gnome.gnome-backgrounds}/share/backgrounds/gnome/blobs-l.svg";
