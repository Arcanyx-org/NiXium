...
# Module exporting configuration from HANA to other systems
...
in mkMerge [
	{
		# SSHD on Onions
		age.secrets.hana-onion = {
			file = ../secrets/hana-onion.age;
			...
			path = "/var/lib/tor/hana-onion.conf";
		...
		services.tor.settings."%include" = [
			config.age.secrets."hana-onion".path
		];
	}
]
