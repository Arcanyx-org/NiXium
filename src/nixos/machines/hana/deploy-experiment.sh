...
targetIP="192.168.0.138"
targetFlake="github:Arcanyx-org/NiXium/experimental#nixos-hana-stable"
...
ssh "root@$targetIP" 'echo 000000 > /run/agenix/hana-disks-password'

ssh "root@$targetIP" 'cat > /etc/ssh/ssh_host_ed25519_key' < <(age -i ~/.ssh/id_ed25519 -d ./src/nixos/machines/hana/secrets/hana-ssh-ed25519-private.age || true)

ssh "root@$targetIP" 'chmod 400 /etc/ssh/ssh_host_ed25519_key'
ssh "root@$targetIP" 'chown root:root /etc/ssh/ssh_host_ed25519_key'

# ssh "root@$targetIP" 'cat > /key' < <(age -i ~/.ssh/id_ed25519 -d ./src/nixos/machines/hana/secrets/hana-unlock-key.age || true)
...
# nix copy --to ssh://root@$targetIP "$(nix build "${targetFlake//#*/}#nixosConfigurations.\"nixos-hana-stable\".config.system.build.toplevel" --print-out-paths || true)"
