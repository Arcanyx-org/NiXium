...
# Deployment:
#     # nix run 'github:nix-community/disko#disko-install' -- --flake 'github:kreyren/nixos-config#hana' --disk system /dev/disk/by-id/ata-WDC_WDS500G2B0A-00SM50_21101J456803
...
let
	...
	diskoDevice = "/dev/disk/by-id/mmc-DF4032_0x535cab07";
	# keyDevice = "/dev/disk/by-id/mmc-SA02G_0x272bcf2e";
	swapSize = "5G";
	impermanentSize = "5G";
in mkMerge [
	{
		age.secrets.hana-disks-password.file = "${self.outPath}/src/nixos/machines/hana/secrets/hana-disks-password.age"; # Supply password for disk encryption
	}
	...
	# 		age.secrets.lengo-unlock-key.file = ../secrets/hana-unlock-key.age; # KeyFile for unlocking the filesystems
						...
						partitions = {
							...
							store = {
								...
								content = {
									...
									passwordFile = config.age.secrets.hana-disks-password.path;
