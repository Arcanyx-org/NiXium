...
# The Management of Automatic Upgrades for HANA

# Credit: https://discourse.nixos.org/t/best-practices-for-auto-upgrades-of-flake-enabled-nixos-systems/31255/2

# FIXME(Krey): Make the machine reboot cleanly each time new update is made to ensure that it's kernel is always up to date
...
in mkIf config.system.autoUpgrade.enable {
	system.autoUpgrade = {
		operation = "switch";
		flake = "path:${self.outPath}#nixos-hana-stable";
		flags = [
