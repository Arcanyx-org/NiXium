...
# Nix-based Disk Management of SCAR with disko and impermenance on tmpfs
...
# Deployment:
#     # nix run 'github:nix-community/disko#disko-install' -- --flake 'github:kreyren/nixos-config#scar' --disk system /dev/disk/by-id/ata-ADATA_SP900_2F4520076497
...
let
	...
	diskoDevice = "/dev/disk/by-id/ata-ADATA_SP900_2F4520076497";
	# keyDevice = "/dev/disk/by-id/mmc-SA02G_0x272bcf2e";
	swapSize = "30G";
	impermanentSize = "8G";
in mkMerge [
	{
		age.secrets.scar-disks-password.file = "${self.outPath}/src/nixos/machines/scar/secrets/scar-disks-password.age"; # Supply password for disk encryption
	}
	...
	# 		age.secrets.lengo-unlock-key.file = ../secrets/scar-unlock-key.age; # KeyFile for unlocking the filesystems
						...
						partitions = {
							...
							store = {
								...
								content = {
									...
									passwordFile = config.age.secrets.scar-disks-password.path;
