...
# Declaration for STABLE release of NixOS for SCAR

let
	inherit (lib) mkForce;
in {
	flake.nixosConfigurations."nixos-scar-stable" = inputs.nixpkgs.lib.nixosSystem {
		system = "x86_64-linux";
		...
		modules = [
			...
			({ pkgs, ... }: {
				...
				# 			"video"
				# 		];
				# 		openssh.authorizedKeys.keys = [ "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHmtDqiOqgXx0WaJE3C+DWCdTegP6vC74/ICAcmA5xja kreyren@scar" ];
				# 	};
	...
	# Task to perform installation of SCAR in NixOS distribution, stable release
	perSystem = { system, pkgs, inputs', self', ... }: {
		packages.nixos-scar-stable-install = pkgs.writeShellApplication {
				name = "nixos-scar-stable-install";
				bashOptions = [
				...
				runtimeEnv = {
					systemDevice = self.nixosConfigurations.nixos-scar-stable.config.disko.devices.disk.system.device;

					secretPasswordPath = self.nixosConfigurations.nixos-scar-stable.config.age.secrets.scar-disks-password.file;

					secretSSHHostKeyPath = self.nixosConfigurations.nixos-scar-stable.config.age.secrets.scar-ssh-ed25519-private.file;
				};
				text = builtins.readFile ./scar-nixos-stable-install.sh;
			};

		# Declare for `nix run`
		apps.nixos-scar-stable-install.program = self'.packages.nixos-scar-stable-install;

		# Unattended installer
		packages.nixos-scar-stable-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
				config.allowUnfree = true;
			};
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-scar-stable-install";
		...
		apps.nixos-scar-stable-unattended-installer-iso.program = self'.packages.nixos-scar-stable-unattended-installer-iso;
	};
}
