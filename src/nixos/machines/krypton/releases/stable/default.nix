...
# Declaration for STABLE release of NixOS for KRYPTON

let
	inherit (lib) mkForce;
in {
	flake.nixosConfigurations."nixos-krypton-stable" = inputs.nixpkgs.lib.nixosSystem {
		...
		modules = [
			self.nixosModules."nixos-krypton"
	...
	# Task to perform installation of krypton in NixOS distribution, stable release
	perSystem = { system, pkgs, inputs', self', ... }: {
		packages.nixos-krypton-stable-install = pkgs.writeShellApplication {
				name = "nixos-krypton-stable-install";
				bashOptions = [
				...
				runtimeEnv = {
					secretSSHHostKeyPath = self.nixosConfigurations.nixos-krypton-stable.config.age.secrets.krypton-ssh-ed25519-private.file;

					derivation = "nixos-krypton-stable";

					machineName = "krypton";
				};
				text = builtins.readFile ./krypton-nixos-stable-install.sh;
			};

		# Declare for `nix run`
		apps.nixos-krypton-stable-install.program = self'.packages.nixos-krypton-stable-install;

		# Unattended installer
		packages.nixos-krypton-stable-unattended-installer-iso = inputs.nixos-generators.nixosGenerate {
			pkgs = import inputs.nixpkgs {
				inherit system;
				config.allowUnfree = true;
			};
			...
			modules = [
				{
					...
					systemd.services.inception = {
						...
						serviceConfig = {
							ExecStart = "${pkgs.nix}/bin/nix run github:NiXium-org/NiXium#nixos-krypton-stable-install";
		...
		apps.nixos-krypton-stable-unattended-installer-iso.program = self'.packages.nixos-krypton-stable-unattended-installer-iso;
	};
}
