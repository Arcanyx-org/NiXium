...
targetIP="10.48.0.110"
targetFlake="github:Arcanyx-org/NiXium/experimental#nixos-sinnenfreude-stable"
...
ssh "root@$targetIP" 'echo 000000 > /run/agenix/sinnenfreude-disks-password'

ssh "root@$targetIP" 'cat > /etc/ssh/ssh_host_ed25519_key' < <(age -i ~/.ssh/id_ed25519 -d ./src/nixos/machines/sinnenfreude/secrets/sinnenfreude-ssh-ed25519-private.age || true)

ssh "root@$targetIP" 'chmod 400 /etc/ssh/ssh_host_ed25519_key'
ssh "root@$targetIP" 'chown root:root /etc/ssh/ssh_host_ed25519_key'

# ssh "root@$targetIP" 'cat > /key' < <(age -i ~/.ssh/id_ed25519 -d ./src/nixos/machines/sinnenfreude/secrets/sinnenfreude-unlock-key.age || true)
...
# nix copy --to ssh://root@$targetIP "$(nix build "${targetFlake//#*/}#nixosConfigurations.\"nixos-sinnenfreude-stable\".config.system.build.toplevel" --print-out-paths || true)"
