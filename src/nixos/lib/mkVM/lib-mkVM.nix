...
###!   mkVM creates a vanilla import: `import inputs.nixpkgs { inherit system; }`.
...
###!
###! system (required)
###!   Guest architecture.  Passed directly to nixosSystem and used to select
###!   the correct QEMU binary.  In flake-parts, this comes from perSystem:
###!     perSystem = { system, pkgs, ... }: mkVM { inherit pkgs system; ... };
###!   Override for cross-architecture VMs (e.g., testing aarch64 on an x86_64 host).
###!   The runner script warns when guest ≠ host, since cross-arch emulation
###!   is slow and some features may not work.
###!
###!   Examples:
###!     system = system;                    # native (from perSystem)
###!     system = "x86_64-linux";            # force x86_64 (for modules requiring it)
###!     system = "aarch64-linux";            # force aarch64
###!
###! ### Display
###!
###! graphical ? null
...
###!
###! system is a required parameter — the guest architecture.  In flake-parts,
###! it comes from perSystem ({ system, pkgs, ... }:).  Override for
...
###!   }
###!
###! Hardware emulation — Quest 3 with custom QEMU flags:
###!   let inherit (self.lib) mkVM; in
###!   { perSystem = { system, pkgs, ... }:
###!       let vm = mkVM {
###!         inherit pkgs system;
...
###! ## NIX REPL
###!
###! mkVM is callable from nix repl for ad-hoc testing without modifying files.
###! This is useful for AI agents and developers who want to iterate quickly:
###!
###!   nix-repl> :lf .
###!   nix-repl> mkVM = lib.mkVM
###!   nix-repl> vm = mkVM {
###!   nix-repl>   pkgs = null;
###!   nix-repl>   system = "x86_64-linux";
###!   nix-repl>   name = "test";
###!   nix-repl>   command = "vim";
###!   nix-repl>   modulePath = "$FLAKE_ROOT/test";
###!   nix-repl>   graphical = "wayland";
###!   nix-repl>   homeManagerModules = [];
###!   nix-repl>   homeManagerConfig = { programs.vim.enable = true; };
###!   nix-repl> }
###!   nix-repl> vm.vm       # «derivation ...> — the raw NixOS VM image
###!   nix-repl> vm.runner   # «derivation ...> — the custom runner script
###!
###! Both vm and runner can be built independently:
###!   nix-repl> :b vm.vm
###!   nix-repl> :b vm.runner
###!
###! ## EXTERNAL FLAKES
###!
###! mkVM is framework-agnostic.  External flakes can use it without flake-parts:
###!
###!   {
###!     inputs = {
###!       nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
###!       nixium.url = "github:Arcanyx-org/NiXium";
###!     };
###!     outputs = { nixpkgs, nixium, ... }:
###!       let
###!         inherit (nixium.lib) mkVM;
###!         system = "x86_64-linux";
###!         pkgs = import nixpkgs { inherit system; };
###!       in {
###!         packages.${system}.my-vm = (mkVM {
###!           inherit pkgs system;
###!           name = "my-test-vm";
###!           command = "bash";
###!           modulePath = "/tmp/my-vm";
###!           graphical = null;
###!           exitMode = "shell";
###!           timeout = null;
###!           homeManagerModules = [];
...
let
		...
		inputs.nixpkgs.lib.nixosSystem {
			...
			modules = [
				...
				home-manager = {
					...
					extraSpecialArgs = { inherit self inputs; } // extraSpecialArgs;
					users.${user} = mkMerge [
	...
	mkVM = {
		...
		let
			...
			runner = mkVmRunner {
				pkgs = resolvedPkgs;
				inherit name system modulePath vmSystem
