...
- `CoderAgent` - Execute individual coding subtasks for non-Nix work (used by BatchExecutor for parallel execution)
- `NixSpecialist` - **ALL Nix/NixOS work** — flake-parts modules, NixOS configs, VM testing, lib/ development. Use for ANY task touching `.nix` files. This agent knows NiXium's architecture and coding standard. Do NOT use CoderAgent for Nix work.
...
> **NiXium rule**: This is a Nix-first codebase. Most tasks will touch `.nix` files. Default to `NixSpecialist` unless the task is purely shell scripts, docs, or non-Nix tooling.
...
<delegation_rules>
  <delegate_when>
    <condition id="nix_work" trigger="any_nix_file" action="delegate_to_nix_specialist">
      ANY task touching .nix files, flake.nix, NixOS modules, lib/ Nix code, or VM testing.
      Do NOT attempt Nix work directly. Do NOT use CoderAgent for Nix.
      NixSpecialist knows flake-parts, the Nx coding standard, and NiXium's architecture.
    </condition>
    <condition id="complex_task" trigger="multi_component_implementation" action="delegate_to_coder_agent">
      For complex, multi-component implementations of non-Nix code, delegate to CoderAgent
    </condition>
    <condition id="security_review" trigger="security_concern" action="delegate_to_security_reviewer">
      Security review, hardening verification, threat modeling, zero-trust compliance checks
    </condition>
  </delegate_when>
  
  <execute_directly_when>
    <condition trigger="simple_implementation">1-4 non-Nix files, straightforward implementation</condition>
    <condition trigger="docs_or_config">Pure documentation, markdown, or non-Nix config edits</condition>
  </execute_directly_when>
</delegation_rules>
